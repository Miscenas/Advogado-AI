    ...
    <div className="flex flex-col items-center gap-6 bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-2xl max-w-lg mx-auto animate-in zoom-in-95">
        ...
        <div className="text-center">
            ...
            <p className="text-sm text-slate-500 leading-relaxed mb-6">
                Para rodar o app no Vercel, o nome da variável de ambiente deve usar <strong>apenas underlines</strong>. O Vercel rejeita hífens em nomes de variáveis.
            </p>
            
            <div className="space-y-4">
                <div className="flex flex-col items-start gap-1">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Configuração Correta (Vercel)</span>
                ...
                <div className="p-4 rounded-2xl bg-indigo-50 border border-indigo-100 text-left">
                    <ul className="text-xs text-indigo-700 space-y-2 list-disc list-inside">
                        <li>Vá em <strong>Settings</strong> > <strong>Environment Variables</strong> na Vercel.</li>
                        <li>Delete a variável antiga se ela tiver hífens.</li>
                        <li>Crie uma nova com Key: <strong>VITE_API_KEY</strong></li>
                        <li>Valor: Sua chave do Gemini (começa com AIza...).</li>
                        <li><strong>IMPORTANTE:</strong> Vá na aba <strong>Deployments</strong> e faça um <strong>Redeploy</strong>.</li>
                    </ul>
                </div>
            </div>
        </div>
             ...
             <div className={`border-2 border-dashed rounded-[2rem] p-16 transition-all duration-500 ${uploadSuccess ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/30'}`}>
                ...
                {isExtracting ? (
                    ...
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-slate-500">Dados importados. Prossiga para revisar os formulários.</p>
                    ...
                    <>
                        ...
                        <h3 className="text-2xl font-bold text-slate-900 mb-2">Análise Automatizada</h3>
                        <p className="text-slate-500 max-w-sm mx-auto mb-10">Envie o documento para preencher o formulário em segundos.</p>
                        <Button className="h-14 px-10 rounded-2xl bg-indigo-600 text-lg shadow-xl shadow-indigo-100" onClick={() => fileInputRef.current?.click()}>Escolher PDF ou Imagem</Button>
                    </>
                )}
             </div>
          </div>
                ...
                {isGenerating ? (
                    <div className="flex flex-col items-center gap-6">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 mt-2">Aplicando fundamentos legais e estruturando pedidos.</p>
                        </div>
                    </div>
                ) : (
                    <>
                        <div className="bg-slate-100 p-6 rounded-[2.5rem] inline-block mb-8">
                            <Scale className="h-16 w-16 text-slate-900" />
                        </div>
                        <h3 className="text-3xl font-bold text-slate-900 mb-4 tracking-tight">Pronto para Revisão</h3>
                        <p className="text-slate-500 mb-10 text-lg max-w-md mx-auto leading-relaxed">Nossa IA Sênior irá gerar o texto completo para sua aprovação final.</p>
                        <Button size="lg" onClick={handleGenerate} className="px-14 h-16 text-xl rounded-2xl shadow-2xl bg-slate-900 shadow-slate-200">
      ...
      default:
        return (
            <div className="space-y-8 animate-in fade-in duration-500">
                  ...
                  <div className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="w-full">
                          ...
                          <select className="flex h-12 w-full rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all shadow-sm cursor-pointer" value={formData.area} onChange={e => handleInputChange('area', e.target.value)}>
                            {AREAS_DO_DIREITO.map(area => (<option key={area.value} value={area.value}>{area.label}</option>))}
                          </select>
                        </div>
                        <Input label="Nomenclatura da Ação" value={formData.actionType} onChange={e => handleInputChange('actionType', e.target.value)} placeholder="Ex: Indenizatória por Danos Morais" className="h-12 rounded-xl" />
                    </div>
                    <Input label="Endereçamento Sugerido" value={formData.jurisdiction} onChange={e => handleInputChange('jurisdiction', e.target.value)} placeholder="Ex: AO JUÍZO DA VARA CÍVEL DA COMARCA DE..." className="h-12 rounded-xl" />
                  </div>
                )}
                {step === 'Fatos' && (
                  <div className="space-y-6">
                     ...
                     <div className="relative group">
                        <textarea className="w-full h-64 p-6 border border-slate-200 rounded-[2rem] focus:ring-2 focus:ring-indigo-500 text-sm shadow-inner transition-all resize-none bg-slate-50/50" value={formData.facts} onChange={e => handleInputChange('facts', e.target.value)} placeholder="Narre os fatos com detalhes técnicos..." />
                        {isTranscribing && (<div className="absolute inset-0 bg-white/70 backdrop-blur-md flex items-center justify-center rounded-[2rem] z-10"><div className="flex flex-col items-center gap-3"><Loader2 className="h-8 w-8 text-indigo-600 animate-spin" /><span className="text-xs font-bold text-slate-900">OUVINDO...</span></div></div>)}
                     </div>
                     <div className="flex gap-4 justify-center">
                        <button onClick={() => toggleRecording('facts')} className={`flex items-center gap-2 px-8 py-4 rounded-2xl shadow-lg transition-all ${isListening ? 'bg-rose-500 text-white animate-pulse' : 'bg-white text-slate-700 hover:bg-slate-50 border border-slate-200'}`}>
                ...
                {step === 'Pedidos' && (
                  <div className="space-y-6">
                     <label className="text-sm font-bold text-slate-700 uppercase tracking-wider">Relação de Pedidos</label>
                     <textarea className="w-full h-48 p-6 border border-slate-200 rounded-[2rem] focus:ring-2 focus:ring-indigo-500 text-sm shadow-inner transition-all resize-none bg-slate-50/50" value={formData.requests.join('\n')} onChange={e => handleInputChange('requests', e.target.value.split('\n'))} placeholder="Liste os pedidos (ex: Tutela antecipada, Danos morais...)" />
                     <Input label="Valor Estimado da Causa" value={formData.value} onChange={e => handleInputChange('value', e.target.value)} placeholder="R$ 0,00" className="h-12 rounded-xl" />
                  </div>
                )}
            </div>
        );
      ...
      <div className="fixed inset-0 z-[200] bg-slate-100 flex flex-col h-screen overflow-hidden">
         <div className="bg-white border-b px-8 py-4 flex justify-between items-center shadow-sm shrink-0">
             <div className="flex items-center gap-4">
                <button onClick={() => setIsFullScreen(false)} className="p-2 text-slate-400 hover:text-slate-900 transition-colors bg-slate-100 rounded-full"><X size={20}/></button>
                <h2 className="text-lg font-bold text-slate-900 tracking-tight">Revisão de Minuta</h2>
             </div>
             <div className="flex gap-3">
                ...
                <Button variant="outline" onClick={() => {
                    ...
                    saveAs(blob, `Petição_${formData.actionType}.doc`);
         ...
         <div className="flex-1 overflow-hidden flex">
             <div className="flex-1 overflow-y-auto p-12 flex flex-col items-center bg-slate-200/50">
                 <div className="w-full max-w-[21cm] bg-white shadow-[0_0_100px_-20px_rgba(0,0,0,0.1)] rounded-sm p-[3cm_2cm_3cm_3cm] h-auto min-h-[29.7cm] outline-none border border-slate-200" contentEditable={true} suppressContentEditableWarning={true} onBlur={e => setGeneratedContent(e.currentTarget.innerHTML)} style={{ fontFamily: '"Times New Roman", serif', fontSize: '12pt', lineHeight: '1.5' }}>
             ...
             <div className="w-96 p-8 bg-white border-l overflow-y-auto shrink-0 flex flex-col gap-6">
                <div className="bg-indigo-50/50 rounded-3xl border border-indigo-100 p-6">
                   <h4 className="text-xs font-bold text-indigo-900 uppercase mb-4 flex items-center gap-2 tracking-widest"><RefreshCw size={14}/> Refinamento Assistido</h4>
                   <textarea className="w-full h-40 rounded-2xl border border-indigo-200 p-4 text-sm mb-4 outline-none focus:ring-2 focus:ring-indigo-300 transition-all bg-white shadow-inner" placeholder="Instrua a IA sobre o que mudar (ex: 'Melhore a fundamentação sobre danos morais' ou 'Adicione o artigo 186 do CC')..." value={refinementText} onChange={e => setRefinementText(e.target.value)} />
                   <Button onClick={async () => {
                   ...
                   }} isLoading={isRefining} className="w-full h-12 bg-indigo-600 hover:bg-indigo-700 rounded-xl font-bold shadow-lg shadow-indigo-100 transition-all">Aplicar Ajuste</Button>
                </div>
             </div>
         </div>
      </div>
    ...
    <div className="max-w-4xl mx-auto py-12">
      {mode === 'selection' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <button onClick={() => { setMode('scratch'); setCurrentStep(1); }} className="bg-white border-2 border-slate-100 p-12 rounded-[3rem] hover:border-indigo-500 hover:shadow-2xl hover:shadow-indigo-100 transition-all h-[400px] flex flex-col items-center justify-center text-center group">
            <div className="bg-indigo-50 p-8 rounded-[2.5rem] group-hover:bg-indigo-100 transition-all group-hover:scale-110 mb-8">
                <PenTool size={64} className="text-indigo-600" />
            </div>
            <h3 className="text-2xl font-bold text-slate-900 tracking-tight">Petição Manual</h3>
            <p className="text-slate-500 mt-3 max-w-xs leading-relaxed">Estruture sua peça do zero com o auxílio da nossa IA sênior.</p>
          </button>
          
          <button onClick={() => { setMode('upload'); setCurrentStep(1); }} className="bg-white border-2 border-slate-100 p-12 rounded-[3rem] hover:border-emerald-500 hover:shadow-2xl hover:shadow-emerald-100 transition-all h-[400px] flex flex-col items-center justify-center text-center group">
            <div className="bg-emerald-50 p-8 rounded-[2.5rem] group-hover:bg-emerald-100 transition-all group-hover:scale-110 mb-8">
            ...
            <p className="text-slate-500 mt-3 max-w-xs leading-relaxed">Envie um arquivo e deixe a IA preencher todos os dados processuais.</p>
          </button>
        </div>
      ) : (
        <div className="bg-white rounded-[3rem] shadow-2xl shadow-slate-200 border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-8 duration-700">
          <div className="bg-slate-50/80 backdrop-blur-md px-10 py-6 border-b flex justify-between items-center">
            ...
            <button onClick={onCancel} className="text-slate-400 hover:text-slate-900 transition-colors bg-white p-2 rounded-full border border-slate-100 shadow-sm"><X size={18}/></button>
          </div>
